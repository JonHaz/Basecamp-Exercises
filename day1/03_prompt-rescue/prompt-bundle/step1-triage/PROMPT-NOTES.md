...
The system prompt for an agent or subagent. Triage an inbound support ticket: assign a P1-P4 priority from business impact alone, and extract only the entities the ticket literally states.
...
python3 scripts/prompt_iterate.py generate \
  --task "Triage an inbound support ticket: assign a P1-P4 priority from business impact alone, and extract only the entities the ticket literally states" \
